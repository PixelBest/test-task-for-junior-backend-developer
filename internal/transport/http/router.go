...
func NewRouter(taskHandler *httphandlers.TaskHandler, docsHandler *swaggerdocs.Handler) *mux.Router {
	...
	api.HandleFunc("/tasks/{id:[0-9]+}/in-progress", taskHandler.SetInProgress).Methods(http.MethodPatch)
	api.HandleFunc("/tasks/{id:[0-9]+}/done", taskHandler.SetDone).Methods(http.MethodPatch)
	api.HandleFunc("/tasks/{id:[0-9]+}/periodic", taskHandler.ClosePeriodicTask).Methods(http.MethodPatch)
